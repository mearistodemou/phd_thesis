...
# 2021-09-27 Fix mplus.plot.histogram, adding na.rm from max and min functions.
# 2021-10-14 Fix mplus.compute.irt.icc where factor and u indices were incorrect accessing loading vector.
# 2021-12-02 Fix access to individual_data/var_names which changed from attribute to dataset.
#
...
mplus.list.variables <- function(file) {
  ...
  var_names <- mplus.get.group.dataset(file, 'individual_data', 'var_names')
...
mplus.plot.histogram <- function(file,v,bins=20) {
  ...
  hist(xx,breaks=seq(min(xx, na.rm=TRUE),max(xx, na.rm=TRUE),length=bins+1),col="red",main=cstr,xlab=var,right=TRUE)
}
